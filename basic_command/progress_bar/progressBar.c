...
void progressbar(int rate){
    ...
    //没有\n 就没有立即刷新，因为显示器默认是行刷新  
    //预留出100s空间，100s 默认是右对齐，进度条是  反 的, 用-100s解决     用单个字符循环覆盖输出实现光标闪动
...
//     while(cnt <= TOP){
//         //没有\n 就没有立即刷新，因为显示器默认是行刷新  
//         printf("[%-100s][%d%%][%c]\r", bar, cnt, label[cnt % len]);    //预留出100s空间，100s 默认是右对齐，进度条是反的, 用-100s解决     用单个字符循环覆盖输出实现光标闪动
//                                                //给进度条跑  %%显示百分号
//         fflush(stdout);
