        ...
        if (*pos == '>')
        {
            ...
            if (*(pos + 1) == '>')
            {
                ...
                *pos++ = '\0';
                ...
                while (isspace(*pos))  // 过滤文件名前的多余空格
                    ++pos;
                redirFileName = pos;
                redirType = APPEND_RDIR;
                break;
            }
            // 输出重定向
            else
...
void interact(char* cLine, int size)
{
    ...
    // 对从标准输入读取的字符串做检查
    check_redir(cLine);
    // 检查后 命令 ls -al > file.txt 变成了 ls -al '\0' file.txt
}
...
void nomalExecute(char* _argv[])
{
    ...
    {
        // 进程程序替换执行命令前，将文件描述符表进行重定向
        if (redirType == IN_RDIR)
        {
            // 输入重定向，从文件中读，因此设置为只读
        ...
        else if (redirType == OUT_RDIR)
        {
            // 输出重定向，设置为 write CREAT TRUNC, 把内容输出到文件中
        ...
        else if (redirType == APPEND_RDIR)
        {
            // 追加重定向，设置为 write CREAT APPEND, 把内容输出到文件中
